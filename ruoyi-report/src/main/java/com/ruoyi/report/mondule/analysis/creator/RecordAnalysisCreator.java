...
import com.ruoyi.report.domain.dto.TextAnalysisMetaData;
 ...
 */
public class RecordAnalysisCreator implements IAnalysisSubTableCreator {
    @Override
    public AnalysisSubTableData create(List<ConfigSubTableCol> configSubTableCols, List<ConfigSubTableRow> configSubTableRows, List<ConfigShift> configShifts, List<Map> dailyList, List<Map> totalList, IAnalysisStrategy strategy) {
        if (configSubTableCols.isEmpty() && configSubTableRows.isEmpty()) {
            ...
            TextAnalysisMetaData metaData = new TextAnalysisMetaData();
            metaData.setText(AnalysisValueUtils.getTodayValueText(dailyList, -1, -1, -1));
            itemList.add(metaData);
            data.add(itemList);
            return new AnalysisSubTableData(newCols, newRows, data, strategy);
        } else if(!configSubTableCols.isEmpty() && configSubTableRows.isEmpty()){
            List<String> newCols = new ArrayList<>();
            List<String> newRows = new ArrayList<>();
            newRows.add("");
            List<List<AnalysisMetaData>> data = new ArrayList<>();
            ...
            for (ConfigSubTableCol configSubTableCol : configSubTableCols) {
                ...
                TextAnalysisMetaData metaData = new TextAnalysisMetaData();
                metaData.setText(AnalysisValueUtils.getTodayValueText(dailyList, -1, configSubTableCol.getId(), -1));
                itemList.add(metaData);
            }
            data.add(itemList);
            return new AnalysisSubTableData(newCols, newRows, data, strategy);
