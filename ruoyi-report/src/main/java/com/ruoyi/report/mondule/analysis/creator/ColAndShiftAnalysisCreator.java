 ...
 */
public class ColAndShiftAnalysisCreator implements IAnalysisSubTableCreator {
    @Override
    public AnalysisSubTableData create(List<ConfigSubTableCol> configSubTableCols, List<ConfigSubTableRow> configSubTableRows, List<ConfigShift> configShifts, List<Map> dailyList, List<Map> totalList, IAnalysisStrategy strategy) {
        ...
        for (int i = 0; i < configShifts.size(); i++) {
            ...
            for (ConfigSubTableCol configSubTableCol : configSubTableCols) {
                ...
                meta.setDailyValue(AnalysisValueUtils.getTodayValue(dailyList, shift.getId(), configSubTableCol.getId(), -1));
                meta.setTotalValue(AnalysisValueUtils.getTotalValue(totalList, shift.getId(), configSubTableCol.getId(), -1));
                itemList.add(meta);
            }
            data.add(itemList);
        }
